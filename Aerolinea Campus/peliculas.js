const peliculasMap = new Map();
    ...
    mensaje = document.getElementById('mensajeEliminar');

  if (peliculasMap.has(titulo)){
    peliculasMap.delete(titulo);
    mensaje.innerHTML = `la película ${titulo} ha sido eliminada correctamente`
  }
  else{
    mensaje.innerHTML = `No existe una pelicula con el titulo ${titulo}`
  }
