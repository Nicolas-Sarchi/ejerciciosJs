...
class CarritoDeCompras {
  constructor() {
    this.elementosCarrito = JSON.parse(localStorage.getItem('elementosCarrito')) || [];
  }
  ...
  mostrarCarrito() {
    ...
    this.elementosCarrito.forEach((item, index) => {
      ...
      elementoCarrito.classList.add('list-group-item', 'd-flex', 'justify-content-between', 'align-items-center');
  ...
  agregarItem(item) {
    if (item){
      ...
      this.mostrarCarrito();
      localStorage.setItem('elementosCarrito', JSON.stringify(this.elementosCarrito));
